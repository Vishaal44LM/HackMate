...
const Room = () => {
  ...
  useEffect(() => {
    const attemptJoin = async () => {
      ...
      if (result.success) {
        ...
        if (result.alreadyMember) {
          toast({
            title: "Welcome back!",
            description: "Reconnected to the room."
          });
        } else {
          toast({
            title: "Joined room",
            description: "You're now part of this ideation session."
          });
        }
      } else {
    ...
    if (!loading && user && roomId && !hasJoined) {
      attemptJoin();
    }
  ...
  const handleLeaveRoom = async () => {
    ...
    toast({
      title: "Left room",
      description: "You've left the ideation room."
    });
    navigate("/dashboard");
  };
  ...
  const ParticipantsList = () => (
    <div className="space-y-2">
      ...
      {Array.from({ length: room.max_participants - activeParticipantsCount }).map((_, i) => (
        <div 
          key={`empty-${i}`}
          className="flex items-center gap-3 p-3 rounded-lg bg-muted/20 border border-dashed border-border"
        >
      ...
      {multiDeviceWarning && !isFocusMode && (
        <div className="bg-amber-500/10 border-b border-amber-500/20 px-4 py-2 flex items-center justify-center gap-2">
          ...
          <p className="text-sm text-amber-600 dark:text-amber-400">
            You are connected from multiple devices; changes are synced.
          </p>
        </div>
      )}
      ...
      <header className={`border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-40 ${isFocusMode ? 'hidden' : ''}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-14 gap-2">
            ...
            {/* Center - User count */}
            <Badge 
              variant={activeParticipantsCount >= room.max_participants ? "destructive" : "secondary"}
              className="shrink-0 gap-1"
            >
              <Users className="w-3 h-3" />
              {activeParticipantsCount}/{room.max_participants}
            </Badge>
            ...
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                className="hidden sm:flex gap-1 text-destructive hover:text-destructive"
              >
                    ...
                    <div className="space-y-3">
                      {suggestions.length === 0 ? (
                        <p className="text-sm text-muted-foreground">
                          Keep chatting to get AI suggestions!
                        </p>
                      ) : (
                        suggestions.slice(0, 3).map((s) => (
                          <div key={s.id} className="p-3 bg-primary/10 rounded-lg">
                            <Badge className="mb-2 text-xs">{s.suggestion_type}</Badge>
                ...
                {messages.length === 0 ? (
                  ...
                  messages.map((msg) => {
                    ...
                    return (
                      <div 
                        key={msg.id} 
                        className={`flex ${isAI ? 'justify-start' : isOwn ? 'justify-end' : 'justify-start'}`}
                      >
                        <div 
                        ...
                        >
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className={`text-xs font-medium ${
                              isOwn && !isAI ? 'text-primary-foreground/80' : 'text-muted-foreground'
                            }`}>
              ...
              <div className="border-t p-4 shrink-0">
                <form onSubmit={handleSendMessage} className="flex gap-2">
                  <Input
                    ...
                    className="flex-1"
                    disabled={sending || !isUserInRoom}
                  />
                  <Button 
                    type="submit" 
                    disabled={sending || !newMessage.trim() || !isUserInRoom}
